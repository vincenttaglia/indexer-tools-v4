...
// Composables
import { useQueryFeesQuery, useSubgraphMetadataMap, useStatusQuery, useEpochQuery, useAllocationsQuery, useColumnPreferences, useOffchainSync } from '@/composables'
...
// Loading state
// ---------------------------------------------------------------------------
const isLoading = computed(() => queryFeesQuery.isLoading.value)

// ---------------------------------------------------------------------------
// Filtering
// ---------------------------------------------------------------------------
const filteredData = computed(() => {
  const all = queryFeesQuery.data.value ?? []
  const search = filterStore.queryFeeFilters.search.toLowerCase().trim()
  if (!search) return all

  return all.filter((row) =>
    row.subgraphDeployment.id.toLowerCase().includes(search),
  )
})
...
// Counts
// ---------------------------------------------------------------------------
const totalCount = computed(() => queryFeesQuery.data.value?.length ?? 0)
...
const columns: ColumnDef<QueryDailyDataPoint, any>[] = [
  ...
  // 4. Avg Gateway Latency (ms)
  columnHelper.accessor('avg_gateway_latency_ms', {
    id: 'avgGatewayLatency',
    header: 'Avg Gateway Latency (ms)',
    size: 200,
    cell: (info) => {
  ...
  // 5. Success Rate
  columnHelper.accessor('gateway_query_success_rate', {
    id: 'successRate',
    header: 'Success Rate',
    size: 130,
    cell: (info) => {
...
const { visibleColumns } = useColumnPreferences('query-fees', columns)
</script>
  ...
  <div class="query-dashboard">
    ...
    <div v-if="!isArbitrum" class="chain-notice">
      <div class="chain-notice-card">
        ...
        <h2>Query Fee Data Not Available</h2>
        <p>Query fee data is only available for <strong>Arbitrum One</strong>. Switch to Arbitrum in settings to view query fee data.</p>
      </div>
    </div>
    ...
    <div class="dashboard-header">
      <div class="header-left">
        <h1 class="page-title">Query Fees</h1>
    ...
    <div class="filter-bar">
      <div class="filter-item filter-search">
        <InputText
          v-model="filterStore.queryFeeFilters.search"
          placeholder="Search deployment IPFS hash..."
          class="filter-input"
        />
      </div>
    </div>
    ...
    <div class="table-wrapper">
      <DataTable
        ...
        :loading="isLoading"
        table-height="100%"
        empty-message="No query fee data found. Ensure you have an API key configured."
      />
    </div>
    </template>
  </div>
